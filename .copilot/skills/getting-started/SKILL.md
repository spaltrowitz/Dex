...
Transform the post-onboarding experience from "blank chat window" to guided value delivery. Adaptive based on what data sources are available (calendar, WorkIQ, or neither).
...
2. Call WorkIQ MCP: `get_recent_meetings(days=7)`
3. Analyze the data
...
Now, want me to:
1. Process those [N] WorkIQ meetings (extract action items)?
...
- WorkIQ: Call `ask_work_iq()` from WorkIQ (ask_work_iq)

Based on results, route to appropriate flow.

---

## Flow A: Has Calendar + WorkIQ
...
2. Analyze WorkIQ meetings from last 7 days
...
Now, want me to process those [N] WorkIQ meetings to extract action items and context?
```
...
[Fetch new WorkIQ meetings since that time]

Since we set up [X hours/days ago]:
• [N] new calendar events
• [M] new WorkIQ meetings
...
Want me to:
1. Process the [M] new WorkIQ meetings?
...
Then execute:
1. Fetch calendar events for this week
2. Analyze WorkIQ data extent using helper function (6 months by default)
...
Here's what I can create from your WorkIQ history:
...
```json
{
  "questions": [{
    "id": "WorkIQ_strategy",
    "prompt": "How would you like to process your WorkIQ data?",
...
1. **Fetch meetings by maximum range needed:**
   ...
   all_meetings = ask_work_iq(days_back=max_days, limit=1000)
   ```
...
## Flow B: Has Calendar OR WorkIQ (Not Both)
...
**But I notice you don't have WorkIQ** - that's how I process meeting transcripts into action items and insights.

Want help with:
1. Installing WorkIQ (automatic meeting intelligence)
...
I can see you have WorkIQ connected. Let me check what's available...

[Analyze WorkIQ data extent - 6 months by default]
...
• Or skip - I can work with just WorkIQ
```
...
## Flow C: Neither Calendar Nor WorkIQ
...
Follow "create MCP server" skill flow:
...
• How to use Copilot to debug APIs
...
```bash
node .copilot/hooks/integration-concierge.cjs
```
...
Want to connect any of these? Or run "integrate tool" anytime later.
```
...
**Discovery:**
• "level up" - Find features you haven't tried
• "integrate tool" - Add more tools anytime
...
elif "process-meetings" not in tried_features and has_WorkIQ:
    # Have WorkIQ but haven't processed
...
After full tour:
- "Remember - invoke "level up" to discover more"
- "Or "integrate tool" to add tools as you need them"
...
- **With data:** Analyzed calendar/WorkIQ intelligently, offered smart actions
...
## Helper: Analyze WorkIQ Data Extent

Before presenting choices, discover how much WorkIQ data is available:

```python
def analyze_WorkIQ_extent(user_email_domain: str, extended: bool = False) -> dict:
    """
    Discover how much WorkIQ data is available
    ...
    result = ask_work_iq(days_back=days_to_fetch, limit=1000)
...
if not extent['has_data']:
    ...
    if extent['has_more_data']:
        ...
        if response['fetch_more'] == 'yes':
            ...
            extent = analyze_WorkIQ_extent(email_domain, extended=True)
